 ...
 */
public class RapidSyncOperation implements Operation, RapidSyncConstants {

    private LogMiner miner = null;

    public RapidSyncOperation(LogMiner miner) {
        this.miner = miner;
    }
        ...
        if (dataList != null && !dataList.isEmpty()) {
            for (int i = input.getStart(); i < dataList.size(); i++) {
                ...
                if (strOper == null || QUERY.equalsIgnoreCase(strOper)) {
                    ...
                    ResultSet result = miner.query(new RapidSyncQuerySQLBuilder(output).generateSelectSQL(elem));
            ...
            return new RapidSyncResult(resultList);
        }
