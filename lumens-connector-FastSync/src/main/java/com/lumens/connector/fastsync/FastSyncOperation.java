...
import com.lumens.connector.fastsync.api.RedoValue;
 ...
 */
public class FastSyncOperation implements Operation, FastSyncConstants {

    private LogMiner miner = null;

    public FastSyncOperation(LogMiner miner) {
        this.miner = miner;
    }
        ...
        if (dataList != null && !dataList.isEmpty()) {
            for (int i = input.getStart(); i < dataList.size(); i++) {
                ...
                if (strOper == null || QUERY.equalsIgnoreCase(strOper)) {
                    ...
                    ResultSet result = miner.query(new FastSyncQuerySQLBuilder(output).generateSelectSQL(elem));
            ...
            return new FastSyncResult(resultList);
        }
