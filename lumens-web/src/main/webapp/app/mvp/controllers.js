...
Lumens.controllers.controller("MainViewCtrl", function($scope, $http, $compile) {
    ...
    $scope.rootLayout = new Lumens.RootLayout($('#id-main-view')).configure();
    $scope.theLayout = new Lumens.SplitLayout($scope.rootLayout).configure({
        mode: "vertical",
        part1Size: 52
    });
    $scope.sysHeader = new Lumens.Header($scope.theLayout.getPart1Element()).setSysTitle("JAMES");
    $scope.navToolbar = new Lumens.NavToolbar($scope.sysHeader.getElement()).configure(Lumens.SysToolbar_Config);
    $scope.navToolbar.onButtonClick(function(event) {
...
.controller("DesignViewCtrl", function($scope, $http, $compile) {
    ...
    if (true)
    {
        $scope.workspaceLayout = new Lumens.SplitLayout($scope.theLayout.getPart2Element()).configure({
            mode: "horizontal",
            part1Size: 220
        });
